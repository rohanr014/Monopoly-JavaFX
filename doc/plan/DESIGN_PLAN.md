...
of the game that we write early on (like app.controller.Engine.Property and app.controller.Engine.Space). Along those same lines, the only open components will be
the larger classes that will be responsible for managing newer features like app.controller.Engine.Board and app.controller.Engine.Player, as we app.controller.Start to add interactions
...
To app.controller.Start our design out, we plan to implement just about everything abstractly, which will give us a good sense on what should
...
interact with the game to prompt those changes, and a app.controller to delegate actions from the user to the model. The game
...
The biggest/main class of our model is the app.controller.Engine.Board class, which will contain the players playing the game (including the
bank, which we are treating as a player since they too are an entity that is simply dishing out money to others), the app.controller.Engine.Space
objects, and the app.controller.Engine.Card objects. The board will be responsible for moving the different players and holding their positions.
...
of app.controller.Engine.Space is app.controller.Engine.CommonSpace, which will represent all the un-ownable spaces on the board, like Chance, Community Chest,
Super Tax, and Go. 
...
players. It will have a callNextPlayer() function to change whoever's turn it is, a move(app.controller.Engine.Player p, app.controller.Engine.Space s) function to
move a player to a specific state, and a move(app.controller.Engine.Player p, int steps) function to move a player a specified number of spaces
to be used after the player rolls their dice. When a player lands on a space, the app.controller.Engine.Space object will call an onLand(app.controller.Engine.Player p)
function to call a set of actions on that player which will change from space to space. For instance, if it is a property
owned by another player, onLand will call that player's giveMoneyTo(double m, app.controller.Engine.Agent a) function to give money to the owner
of that property. Each card that can get picked up will also invoke an action on the player that picks it up. 

---

# User Interface
**This section describes what components the user will interact with in your program (keep it simple to app.controller.Start) and how a game 
...
The game is primarily interacted with through the app.controller.Engine.Board class, which has instances of every object in the 
...
there's a new type of app.controller.Engine.Space that has different functions it's very easy to slot in a new class in the hierarchy.
...
app.controller.Engine.Board - contains the players playing the game (including bank, which we are treating as a player since they too are an 
entity that is simply dishing out money to others) as well as the game's dice, the app.controller.Engine.Space and app.controller.Engine.Card objects. The board
is primarily responsible for moving players, holding their positions, and regulating turn order.

app.controller.Engine.Card - (abstract) app.controller.Engine.Card that invokes an action                

app.controller.Engine.ImmediateCard - Invokes an action immediately when drawn

app.controller.Engine.HoldableCard - can be stored and used later

app.controller.Engine.Space - (Abstract) Superclass for all spaces on board 

app.controller.Engine.CommonSpace - Spaces like Chance/CommunityChest/Jail/FreeParking, etc that are not properties

---

# Design Details (cont.)

app.controller.Engine.Property - Superclass for all types of properties

app.controller.Engine.SetProperty - Class for Utilities and Railroads, properties who's rent is a function of how many of the set the player owns

app.controller.Engine.ColorProperty - Subclass of app.controller.Engine.SetProperty, consists of the colored properties that can be built on once a monopoly is reached

app.controller.Engine.Agent - abstract class for all game agents. May eventually include CPUs

app.controller.Engine.Bank - Essentially a player with a wallet who never acts

app.controller.Engine.Player - app.controller.Engine.Player of game with full functionality to play (buy/sell, mortgage, build, everything)
...
Funds - Handled in app.controller.Engine.Agent classes, accessible only through agents
Dice - Handled in app.controller.Engine.Board, extendable through Boards' interpretations of RNG
...
Another game can be vanilla monopoly, but with rules modifications such as all the properties being dealt on app.controller.Start, double
...
certain design headaches regarding the app.controller.Engine.Player's access to and knowledge of their own money.
